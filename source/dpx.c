...
static int read_dpx_image_data(FILE *fp, pic_t **p, int orientation, int sign, int bpp, int descriptor, int rle, int bugs, int w, int h, int bswap);
...
{
	return write_dpx_ver(fname, p, p->ar1, p->ar2, p->frm_no, p->seq_len, p->framerate, p->interlaced, p->bits, STD_DPX_VER, pad_line_ends, bswap);
}
...
{
	return write_dpx_ver(fname, p, ar1, ar2, frameno, seqlen, framerate, interlaced, bpp, STD_DPX_VER, pad_line_ends, bswap);
}

int write_dpx_ver(char *fname, pic_t *p, int ar1, int ar2, int frameno, int seqlen, float framerate, int interlaced, int bpp, int ver, int pad_line_ends, int bswap)
{
	FILE *fp;
	int i, j, k, xindex, b;
	DPXFILEFORMAT f;
	...
	sprintf(f.FileHeader.Copyright, "(C) Broadcom Corp");
	...
	if((bpp == 8) || (bpp == 16))
		...
		f.ImageHeader.ImageElement[0].Packing = READ_DPX_16(1);   /* Method A */
	else if(bpp == 12)
		f.ImageHeader.ImageElement[0].Packing = READ_DPX_16(2);   /* Method B */
...
#define BYTE_SWAP(x)  x = (((x & 0xff) << 24) | ((x & 0xff00) << 8) | ((x & 0xff0000) >> 8) | (x >> 24))
	...
	if (ver == STD_DPX_VER) // DPX standard 2.0
	{
		if (p->color == RGB)
		{
			nbuffer = 1;
			...
			for (i = 0; i < hbuff[b]; ++i) // every row
			{
				for (j = 0; j<(subsampled[b] ? wbuff[b]/2 : wbuff[b]); ++j) // every pixel
				{
					for (k=0; k<ndatum[b]; ++k)
					{
						...
						case 12:
						case 16:
							data32b |= (datum[b][k][i][xindex] << (element<<4)); // Offset by 16 bits
							element = (element+1) % 2; // 2 datums = 2*16 = 32bits per D-word
							break;
						case 10:
							data32b |= (datum[b][k][i][xindex] << (element*10+2));
							element = (element+1) % 3;
							break;
						case 8:
							data32b |= (datum[b][k][i][xindex] << (element<<3));
							element = (element+1) % 4;
							break;
						default:
							return(DPX_ERROR_UNSUPPORTED_BPP);
		...
		fclose(fp);
		return(0);
	} // end of writting DPX standard 2.0
...
{
	int high, low;
	int ret_val = dpx_read_hl(fname, p, &high, &low, dpx_bugs);
	if (!ret_val) {
		*ar1        = (*p)->ar1;
		*ar2        = (*p)->ar2;
		*frameno    = (*p)->frm_no;
...
{
	int high, low;
	return dpx_read_hl(fname, p, &high, &low, dpx_bugs);
}

int dpx_read_hl(char *fname, pic_t **p, int* highdata, int*lowdata, int dpx_bugs)
{
	FILE *fp;
	DWORD magic;
	int bswap = 0;
	...
	fseek(fp, 0, SEEK_SET);
	fread(&f, sizeof(DPXFILEFORMAT), 1, fp);

	w = READ_DPX_32(f.ImageHeader.PixelsPerLine);
	h = READ_DPX_32(f.ImageHeader.LinesPerElement);
	*p = NULL;

	if (strcmp(f.FileHeader.Project, "bcmdpx1.00")==0)
	{
		bugs = 1;
	} else if (strcmp(f.FileHeader.Project, "bcmdpx1.00 DVS")==0)
	{
		bugs = 2;
	} else if (strcmp(f.FileHeader.Creator, "DVS Clipster")==0)
	{
		bugs = 2;
	}
	if(dpx_bugs>=0)		// Bugs override
		bugs = dpx_bugs;  
	...
	for (i=0; i<READ_DPX_16(f.ImageHeader.NumberElements); ++i)
	{
		...
		ecode = read_dpx_image_data(fp, p, READ_DPX_16(f.ImageHeader.Orientation), READ_DPX_32(f.ImageHeader.ImageElement[i].DataSign),
			...
			READ_DPX_16(f.ImageHeader.ImageElement[i].Encoding), bugs, w, h, bswap);
		if (ecode)
		{
			return(ecode);
		}
			...
			for (j = 0; j < (subsampled_x[b] ? wbuff[b]/2 : wbuff[b]); ++j)
			{
				for (k = 0; k < ncomponents[b]; ++k)
				{
					xindex = (subsampled_x[b] && (subs[b][k] == 1))? j*2
						...
						: j;
					if (!bugs) // read as DPX 2.0 compatible files
					{
						if (nelement == 0)
						{
							fread(&data32b, 4, 1, fp);
							if (bswap)
							{
								BYTE_SWAP(data32b);
							}
						...
						case 8:
							...
							{
								if(!skip_read[k])
									ptr[b][k][i][xindex] = (char) ((data32b >> (nelement*8)) & 0xff);
							} else
							{
								if(!skip_read[k])
									ptr[b][k][i][xindex] = (data32b >> (nelement*8)) & 0xff;
							}
							nelement = (nelement+1) % 4;
							break;
						case 10:
							if (sign)
							{
	 						...
	 						if(!skip_read[k])
 								ptr[b][k][i][xindex] = (data32b >> (2+10*nelement)) & 0x3ff;
						...
						case 16:
							...
							{
								if(!skip_read[k])
									ptr[b][k][i][xindex] = (char) ((data32b >> (nelement*16)) & 0xffff);
							} else
							{
								if(bugs==3)
								{
									if(!skip_read[k])
										ptr[b][k][i][xindex] = (data32b >> ((1-nelement)*16)) & 0xffff;
								}
								else
									if(!skip_read[k])
										ptr[b][k][i][xindex] = (data32b >> (nelement*16)) & 0xffff;
							}
							nelement = (nelement+1) % 2;
							break;
						default:
							return(DPX_ERROR_NOT_IMPLEMENTED);
						}
					} else // DVS compatible. bug == 2
					{
						switch (bpp)
						{
						case 8:
							...
							{
								if(!skip_read[k])
									ptr[b][k][i][xindex] = (char)elements[nelement];
							} else
							{
								if(!skip_read[k])
									ptr[b][k][i][xindex] = elements[nelement];
							}
							nelement = (nelement+1) % 4;
							break;
						case 10:  /* Assumes method A */
							if (sign)
							{
								return(DPX_ERROR_NOT_IMPLEMENTED);
							}
							if (nelement == 0)
							{
								if ((bugs==2) && ((*p)->chroma==YUV_422))
								{
									elements[3] = fgetc(fp);
									elements[2] = fgetc(fp);
									elements[1] = fgetc(fp);
									elements[0] = fgetc(fp);
								} else if (bugs==2)  // (*p)->chroma==YUV_444 (aka 444 RGB or 444 YUV)
								{
									// machine endian
									fread(&data32b, 4, 1, fp);
									if (bswap)
							...
							{
							case 2:
								d = ((elements[0] & 0xff) >> 2) | ((elements[1] & 0xf) << 6);
								break;
							case 1:
								d = ((elements[1] & 0xf0) >> 4) | ((elements[2] & 0x3f) << 4);
								break;
							case 0:
								d = ((elements[2] & 0xc0) >> 6) | ((elements[3] & 0xff) << 2);
								break;
							}
							nelement = (nelement+1) % 3;
							if(!skip_read[k])
